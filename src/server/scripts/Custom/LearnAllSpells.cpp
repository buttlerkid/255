...
class LearnSpellsOnLevelUp : public PlayerScript
{
	public:
		...
		{ 
			uint32 temp[] = {
				...
				52372, 49142, 52375, 47633, 47632, 52373, 50536
							};
	...
	void LearnSpellsForNewLevel(Player* player, uint8 level)
	{
		...
		for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i)
		{
			...
			{
				SpellInfo const* tempSpell = spellInfo;
				for (uint8 j = spellInfo->GetRank(); j > 0; j--)
				{
					if (GetTalentSpellPos(tempSpell->Id))
					{
						valid = false;
						break;
					}
					tempSpell = tempSpell->GetPrevRankSpell();
				}
			}
			...
			if (spellInfo->BaseLevel == level && sSpellMgr->IsSpellValid(spellInfo, player) && valid)
